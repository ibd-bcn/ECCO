...
# Prepare for GET ####
# perl ~/Documents/projects/GETS/gets.pl --matrix=./processed/new.tsv --geneinfo=./processed/gene_info_new.tsv --sampleinfo=./processed/STEM_samples_info_new.tsv --colors=./processed/colors_info_new.tsv --output=./processed/STEM_GETS_new --center=TRUE --overwrite=TRUE
...
write.table(gene_info2[, -1], "processed/gene_info_new_sigma.tsv", sep = "\t", row.names = FALSE,
...
meta_o %>%
    ...
    t() %>%
    write.table(file = "processed/samples_info_new_sigma.tsv", sep = "\t",
                row.names = TRUE, quote = FALSE, col.names = FALSE, na = "")
meta_o %>%
    dplyr::filter(colname %in% colnames(stem)[-1]) %>%
    dplyr::select(-colname) %>%
    t() %>%
    write.table(file = "processed/STEM_samples_info_new_sigma.tsv", sep = "\t",
                row.names = TRUE, quote = FALSE, col.names = FALSE, na = "")
meta_o %>%
    dplyr::filter(colname %in% colnames(diff)[-1]) %>%
    dplyr::select(-colname) %>%
    t() %>%
    write.table(file = "processed/DIFF_samples_info_new_sigma.tsv", sep = "\t",
                row.names = TRUE, quote = FALSE, col.names = FALSE, na = "")
...
data.frame(pos = "SAMPLEINFO",
    ...
    write.table(file = "processed/colors_info_new_sigma.tsv", quote = FALSE, sep = "\t",
