...
info$CD_endoscopic_response <- as.character(info$CD_endoscopic_response)

info[info$CD_endoscopic_response=="1" & !is.na(info$CD_endoscopic_response),"CD_endoscopic_response"]  <- "no"
info[info$CD_endoscopic_response=="2" & !is.na(info$CD_endoscopic_response),"CD_endoscopic_response"]  <- "no"
info[info$CD_endoscopic_response=="3" & !is.na(info$CD_endoscopic_response),"CD_endoscopic_response"]  <- "yes"

info[info$UC_endoscopic_response=="1" & !is.na(info$UC_endoscopic_response),"UC_endoscopic_response"]  <- "no"
info[info$UC_endoscopic_response=="2" & !is.na(info$UC_endoscopic_response),"UC_endoscopic_response"]  <- "no"
info[info$UC_endoscopic_response=="3" & !is.na(info$UC_endoscopic_response),"UC_endoscopic_response"]  <- "yes"
...
############################################################################################
########## Change names to official HGNC symbol in RNA-seq data whenever possible ##########
############################################################################################

write.table(rownames(RNAseq.data.all), "/home/daguilar/scripts/expression/rownames.txt", sep="\t", row.names=FALSE, col.names=FALSE, quote=FALSE)

rownames.new <- system("perl /home/daguilar/scripts/expression/find_official_gene_names2.pl /home/daguilar/scripts/expression/rownames.txt /home/daguilar/HGNC/hgnc_complete_set.tsv /home/daguilar/references/hg19/genes.gtf 0", intern=TRUE)

if (sum(duplicated(rownames.new)) > 0)
	{stop("[     ERROR] Duplicated gene names: ", paste(rownames.new[duplicated(rownames.new)], collapse=" "),"\n");}

rownames(RNAseq.data.all) <- rownames.new
