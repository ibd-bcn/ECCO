...
linear_model <- linear_model[1:19]
pdf("plots/compare_techs.pdf")
for (i in seq_along(linear_model)) {
    lm_values <- linear_model[[1]]$logFC
    ...
    print(sum(lm_values == s_samples))
}
dev.off()

plot(linear_model[[1]]$AveExpr, linear_model[[1]]$logFC, pch = ".")
