...
# Design ####
grouping <- apply(meta[, c("cell_type", "TYPE"), drop = FALSE], 1,
                  paste0, collapse = "_")
...
write.table(gene_info2[, -1], "processed/gene_info_old_ileum.tsv", sep = "\t", row.names = FALSE,
...
meta_o %>%
    ...
    t() %>%
    write.table(file = "processed/samples_info_old_ileum.tsv", sep = "\t",
                row.names = TRUE, quote = FALSE, col.names = FALSE, na = "")
meta_o %>%
    dplyr::filter(colname %in% colnames(stem)[-1]) %>%
    dplyr::select(-colname) %>%
    t() %>%
    write.table(file = "processed/STEM_samples_info_old_ileum.tsv", sep = "\t",
                row.names = TRUE, quote = FALSE, col.names = FALSE, na = "")
meta_o %>%
    dplyr::filter(colname %in% colnames(diff)[-1]) %>%
    dplyr::select(-colname) %>%
    t() %>%
    write.table(file = "processed/DIFF_samples_info_old_ileum.tsv", sep = "\t",
                row.names = TRUE, quote = FALSE, col.names = FALSE, na = "")
...
data.frame(pos = "SAMPLEINFO",
    ...
    write.table(file = "processed/colors_info_old_ileum.tsv", quote = FALSE, sep = "\t",
